				...
				<div class="content">
					...
					<div class="panel panel-flat col-sm-4">
							<div class="panel-heading">
								<h5 class="panel-title">Question No. <span id="IndexQues"></span> :</h5>
							...
							<div class="panel-body" style="min-height:250px;">
                                   ...
                                   <input type="hidden" id="exam_id" value="<?=$getexam['id'];?>"/>
                        ...
                        <div class="panel panel-flat col-sm-4">
                            ...
                            <?php $r=1; $a=1; foreach($questionList as $sub=>$question){?>
								<div class="panel panel-white">
									<div class="panel-heading">
										<h6 class="panel-title ">
                                        
											<a data-toggle="collapse" data-parent="#accordion-control-right" href="#accordion-control-right-group<?=$r;?>"><?=$sub;?></a>								
										</h6>
									</div>
									<div id="accordion-control-right-group<?=$r;?>" class="panel-collapse collapse <?=$r==1?'in':'';?>">
										<div class="panel-body">
                                        <?php for($i=0; $i < count($question); $i++){?>
											<a onClick="getquesbyids(<?=$question[$i]['id'];?>, <?=$question[$i]['sub_id'];?>, <?=$a;?>)" id="<?=$a;?>"><span id="bg<?=$a;?>" class="badge bg-primary" style="margin:5px;"><?=$a;?></span></a>
...
function getformdata(){
	...
	sessionStorage.setItem(question_id,answere);
...
function skipforreviewQuestion(){
	...
	localStorage.setItem(question_id, answere)
	loadQuestionbySNo(lastunattemp);
	}
	else{
		loadQuestionbySNo(lastunattemp);
...
function getquesbyids(ques_id, sub_id, btnid){
	//alert(ques_id+'-'+sub_id+'-'+btnid);
	//console.log($.ajax);
	if(btnid == '1'){
	...
	if($Question_mode == 'attempted'){
		...
		if(getans !=null){
		$('#'+getans).prop('checked', true).uniform('refresh');
		$('#skipButton').hide();
...
function final_submission(){
	...
	$.ajax({
                type      : 'POST',
                url       : '<?php echo base_url('secure/exam/getdemoResult');?>',
				data	  : 'data='+data,
                dataType  :'JSON',
                success   : function(response){
					console.log(response);
					var url = '<?php echo base_url('secure/exam/showdemoResult');?>'+'/'+response.rowid;
